    ...
    <section className="lk-band bg-[var(--lk-bg)] text-[var(--lk-ink)]">
      <div className="mx-auto max-w-[720px] text-center">
        <FadeIn>
          ...
          <p className="mx-auto mt-4 max-w-lg text-[var(--lk-muted)]">
            {LOUPKIDS_OFFER.scarcityLine}. {LOUPKIDS_OFFER.callingCanonical}
          </p>

          <div className="lk-card mt-8 text-left">
            <p className="text-sm font-medium text-[var(--lk-ink)]">{HOME_LAUNCH.scarcityLine}</p>
            ...
            <p className="mt-3 text-sm font-medium leading-snug">{LOUPKIDS_OFFER.callingCanonical}</p>
            <ul className="mt-5 space-y-2 text-sm leading-snug text-[var(--lk-muted)]">
              {HOME_LAUNCH.bullets
                .filter((b) => !b.startsWith("Loup↔Loup"))
                .map((b) => (
                  <li key={b}>{b}</li>
                ))}
            </ul>
