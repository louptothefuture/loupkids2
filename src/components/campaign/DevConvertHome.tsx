...
function ConvertCta({ className = "", dark = false }: { className?: string; dark?: boolean }) {
  ...
  return (
    <div className={`flex flex-col items-center gap-3 ${className}`}>
      <Link href="/shop/loup" className={btnClass}>
        {LOUPKIDS_CTA.primary}
      </Link>
      <p className={`flex flex-wrap items-center justify-center gap-x-1.5 text-pretty text-sm ${textClass}`}>
        <span aria-hidden="true">🛡️</span>
        <span>
          <strong className={strongClass}>{LOUPKIDS_GUARANTEE.title}</strong>
          {" — "}
      ...
      <section className="border-b-2 border-ink bg-white pb-8 pt-6 sm:pb-12 sm:pt-10">
        <div className="mx-auto grid max-w-5xl items-center gap-8 px-4 sm:px-6 lg:grid-cols-2 lg:gap-12">
          <Reveal>
            ...
            <div className="mt-8">
              <ConvertCta className="!items-start sm:!items-start" />
            </div>
          </Reveal>
          <Reveal delay={0.08}>
